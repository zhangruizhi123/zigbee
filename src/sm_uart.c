...
static ssize_t safe_write(int fd, const void *vptr, size_t n)
{
    size_t  nleft;
    ssize_t nwritten;
    const char *ptr;
...
static int uart_read(int fd,char *r_buf,size_t len)
{
    ssize_t cnt = 0;
    fd_set rfds;
    struct timeval time;
...
static int uart_write(int fd,const char *w_buf,size_t len)
{
    ssize_t cnt = 0;
...
static void uart_create_thread(void*data)
{
    uart_callback call=(uart_callback) data;
    char buff[512]={0};
    int ret=0;
    while(1)
    {
...
{
    fd = uart_open(fd,name);/*串口号/dev/ttySn,USB口号/dev/ttyUSBn*/
    if(fd == -1)
    {
        fprintf(stderr,"uart_open error\n");
        return -1;
    }
    if(uart_set(fd,speed,0,8,'N',1) == -1)
    {
...
int sm_uart_callback(uart_callback call)
{
    ...
    int err = pthread_create(&tid1, NULL, (void*)uart_create_thread, (void*)call);
    if(err!=0)
    {
